...
template <typename> class basic_address_iterator;   //先制声明

/// Type used for storing IPv6 scope IDs.
typedef uint_least32_t scope_id_type;   // 【scope_id也是u32】
...
public:
...
#else
  typedef asio::detail::array<unsigned char, 16> bytes_type;    //相比之下ipv6的就是 u8*16 = 128bit
#endif
  ...
  /// The scope ID of the address.【返回scope_id】
  /**
  ...
  /// The scope ID of the address. 【传入并设置scope_id】
  /**
  ...
  /// Determine whether the address is a loopback address. 【判断是否是回环地址】
  ASIO_DECL bool is_loopback() const ASIO_NOEXCEPT;

  /// Determine whether the address is unspecified. 【判断地址是否没有指定::/128】
  ASIO_DECL bool is_unspecified() const ASIO_NOEXCEPT;

  /// Determine whether the address is link local.  【判断地址是否是本地链路地址  ::1/128 fe80::/10】
  ASIO_DECL bool is_link_local() const ASIO_NOEXCEPT;

  /// Determine whether the address is site local.  【判断地址是否是唯一区域地址  fc00::/7】
  ASIO_DECL bool is_site_local() const ASIO_NOEXCEPT;

  /// Determine whether the address is a mapped IPv4 address.【判断是否是ipv4转译地址 ::ffff:x.x.x.x/96 2001::/32 2002::/16】
  ...
  /// Determine whether the address is a multicast address. 【是否是多播地址FF00::/8】
  ASIO_DECL bool is_multicast() const ASIO_NOEXCEPT;

  /// Determine whether the address is a global multicast address. 【FF02::1:FF00:0/104】
...
private:
  friend class basic_address_iterator<address_v6>;  //为迭代器声明友元类【因为要访问里面的私有数据吧】【与v4的实现相比有所不同，v4里面iterator++是重新构造了一个新的地址object，而这里v6就只是更改了address_里的私有数据，或许作者在实现的时候有什么别的想法吧】

  // The underlying IPv6 address.   【地址in6_addr 别名[联合体u8*16或者u16*8]】
  asio::detail::in6_addr_type addr_;

  // The scope ID associated with the address. 【scope_id】
...
struct hash<asio::ip::address_v6>
{
  std::size_t operator()(const asio::ip::address_v6& addr)
    const ASIO_NOEXCEPT
  {
    const asio::ip::address_v6::bytes_type bytes = addr.to_bytes(); //拿到v6的u8*16数组
    std::size_t result = static_cast<std::size_t>(addr.scope_id()); //拿到scope_id【本来是u32的，转成了u32/u64】
