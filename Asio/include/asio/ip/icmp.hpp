...
{
public:
  /// The type of a ICMP endpoint. 【icmp端点信息】
  typedef basic_endpoint<icmp> endpoint;

  /// Construct to represent the IPv4 ICMP protocol.【生成v4的ICMP实例】
  ...
  /// Obtain an identifier for the protocol. 【返回协议族】
  int protocol() const ASIO_NOEXCEPT
  {
    return protocol_;
  }

  /// Obtain an identifier for the protocol family. 【返回地址族】
  int family() const ASIO_NOEXCEPT
  {
    return family_;
  }

  /// The ICMP socket type. 【基本socket类模板<icmp>别名】
  typedef basic_raw_socket<icmp> socket;    

  /// The ICMP resolver type. 【基本解析类模板<icmp>别名】
  typedef basic_resolver<icmp> resolver;

  /// Compare two protocols for equality. 【判断是不是一样的，就去看其协议族和地址族是不是一样的】
...
private:
  ...
  int protocol_;    //协议族IPPROTO_ICMP/IPPROTO_ICMP6
  int family_;  //地址族AF_INET/AF_INET6
};
