...
{
public:
  /// The type of the elements pointed to by the iterator. 【迭代器里面有一个ipv4地址类 别名】
  typedef address_v4 value_type;

  /// Distance between two iterators. 【迭代器之间的距离差，基本单位为指针64位就是int64,32位就是int 别名】
  typedef std::ptrdiff_t difference_type;  

  /// The type of a pointer to an element pointed to by the iterator. 【指向迭代器的指针 别名】
  typedef const address_v4* pointer;

  /// The type of a reference to an element pointed to by the iterator. 【指向迭代器的引用 别名】
  typedef const address_v4& reference;

  /// Denotes that the iterator satisfies the input iterator requirements. 【】
  typedef std::input_iterator_tag iterator_category;

  /// Construct an iterator that points to the specified address. 【初始化函数，初始化private address_】【最开始传入的是地址对象的引用】
  ...
  /// Move constructor. 【采用static_cast<type&&>】
  ...
  /// Assignment operator. 【赋值运算直接拷贝就行[因为address的实例实现了自己的赋值运算函数]】
  ...
  /// Pre-increment operator. 【++i的重定义就是地址对应的u32+1[循环]】
  ...
  /// Post-increment operator.  【i++定义】
  basic_address_iterator operator++(int) ASIO_NOEXCEPT
  {
    basic_address_iterator tmp(*this); // 先使用拷贝构造去构造临时对象
    ++*this;
    return tmp; // 返回临时对象
  }

  /// Pre-decrement operator. 【--i同理，循环的】
  basic_address_iterator& operator--() ASIO_NOEXCEPT
  {
    address_ = address_v4((address_.to_uint() - 1) & 0xFFFFFFFF);
    return *this;
  ...
  /// Post-decrement operator. 【i--同理，先拷贝构造临时对象】
  ...
  /// Compare two addresses for equality.   【比较引用的地址大小】
