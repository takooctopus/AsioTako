...
class endpoint //UNIX domain endpoint的帮助类
{
public:
  // Default constructor.   【下面有三种带path_name的构造函数】
  ASIO_DECL endpoint();
  ...
  // Get the underlying endpoint in the native type.【返回里面实例的地址】
  asio::detail::socket_addr_type* data()
  {
    return &data_.base; 
  }

  // Get the underlying endpoint in the native type. 【返回里面实例[因为是本地的，所以就是path]的地址】
  const asio::detail::socket_addr_type* data() const
  {
    return &data_.base;
  }

  // Get the underlying size of the endpoint in the native type.【获取其总数据大小 2 + path_length_】
  ...
  {
    return sizeof(asio::detail::sockaddr_un_type); //110byte
  }
...
private:
  ...
  union data_union
  {
    asio::detail::socket_addr_type base;    //要嘛是普通的【这个ipv4的sockaddr只有16byte】
    asio::detail::sockaddr_un_type local;   //要嘛是本地的【本地的这个更大一共有2+108=110byte】
  } data_;  //私有的数据结构UNION，用来存放网络的或者本地的端点信息

  // The length of the path associated with the endpoint.
  std::size_t path_length_; //用一个u64来表示地址长度
