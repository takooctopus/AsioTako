...
{
public:
  /// Construct a protocol object for a specific address family and protocol. 【传入地址族和套接字协议】
  ...
  {
    if (source_protocol.type() != type())   //注意，套接字协议得相同
    {
      std::bad_cast ex;
  ...
  /// Obtain an identifier for the type of the protocol. 【对于这个模板类的特例，返回套接字类型 => SOCK_STREAM】
  int type() const ASIO_NOEXCEPT
  {
    return ASIO_OS_DEF(SOCK_STREAM);    
  }

  /// Obtain an identifier for the protocol. 【套接字协议 => IPPROTO_TCP/UDP】
  int protocol() const ASIO_NOEXCEPT
  {
    return protocol_;
  }

  /// Obtain an identifier for the protocol family. 【地址族 AF_INET/INET6】
  ...
  /// The type of an endpoint. 【特例化基本端点类】
  typedef basic_endpoint<stream_protocol> endpoint;

  /// The generic socket type. 【特例化基本流式套接字类】
  typedef basic_stream_socket<stream_protocol> socket;

#if !defined(ASIO_NO_IOSTREAM)
  /// The generic socket iostream type. 【特例化基本iostream类】
