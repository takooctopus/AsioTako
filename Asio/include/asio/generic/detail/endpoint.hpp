...
// Helper class for implementing a generic socket endpoint. 【通用的socket端点】
class endpoint
{
public:
  // Default constructor.
  ...
  // Construct an endpoint from the specified raw bytes. 【传入sock_addr和它的长度进行构造，其实是调用init直接拷贝的，这个相当于封装了一系列函数】
...
private:
  // The underlying socket address.【先看基本数据类型，是一个联合体，sockaddr/sockaddr_storage】
  union data_union
  {
    asio::detail::socket_addr_type base; //【基本的sockaddr 16byte】
    asio::detail::sockaddr_storage_type generic; //【基本的sockaddr_storage 128byte】
  } data_;  

  // The length of the socket address stored in the endpoint. 【在端点中的地址大小】
  std::size_t size_;

  // The socket protocol associated with the endpoint. 【在端点中的协议族】
  int protocol_;

  // Initialise with a specified memory. 【初始化函数，用一个sock_addr进行初始化】
