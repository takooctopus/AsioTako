...
typedef std::error_category error_category; //一样，使用typedef来进行类的多态实现

#else // defined(ASIO_HAS_STD_SYSTEM_ERROR)

/// Base class for all error categories.
class error_category : private noncopyable  //error不能被拷贝
{
public:
  /// Destructor.
  ...
  /// Returns a string naming the error gategory. 【名称】
  virtual const char* name() const = 0;

  /// Returns a string describing the error denoted by @c value. 【错误消息[int]】
  virtual std::string message(int value) const = 0;

  /// Equality operator to compare two error categories. 【错误类型比较】
  bool operator==(const error_category& rhs) const
  {
    return this == &rhs;
  }

  /// Inequality operator to compare two error categories. 【错误类型比较】
  bool operator!=(const error_category& rhs) const
  {
    return !(*this == rhs);
  }
...
public:
  ...
  /// Construct with specific error code and category.【传入】
