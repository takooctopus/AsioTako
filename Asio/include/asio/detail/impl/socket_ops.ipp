...
extern "C" char* if_indextoname(unsigned int, char*);   //【链接】根据scope_id获取网卡名称
extern "C" unsigned int if_nametoindex(const char*);    //【链接】根据网卡名称获取scope_id
    ...
    unsigned long scope_id, asio::error_code& ec)
{
  ...
  if (af == ASIO_OS_DEF(AF_INET))
  {
    sprintf_s(dest, length, "%u.%u.%u.%u",
        bytes[0], bytes[1], bytes[2], bytes[3]);    // 【ipv4用sprintf_s去格式化u8=>转化成10进制整数】
    return dest;
  }
  ...
  union
  {
    socket_addr_type base;          //【u16 sa_family, u8*14 sa_data】 = 16 byte
    sockaddr_storage_type storage;  //【u16 ss_family, u8*(8-2) __ss_pad1, __int64 __ss_align, u8*() __ss_pad2】=> 128 byte
    sockaddr_in4_type v4;   // 【u16 sin_family, u16 sin_port, u8*4 in_addr, u8*8 sin_zero[8]】 => 16 byte
    sockaddr_in6_type v6;   // 【u16 sin6_family, u16 sin6_port, u32 sin6_flowinfo, u8*16 in6_addr, u32 sin6_scope_id/sin6_scope_struct】 => 28byte
  } address;    // address的几种结构，当然我们一般还是先定义storage这个再强转
  DWORD address_length; //定义一个u64的长度
  if (af == ASIO_OS_DEF(AF_INET))
  {
    // 对ipv4来说
    address_length = sizeof(sockaddr_in4_type); //采用的结构体16byte
    address.v4.sin_family = ASIO_OS_DEF(AF_INET);   //协议u16
    address.v4.sin_port = 0;    //端口设置为0
    memcpy(&address.v4.sin_addr, src, sizeof(in4_addr_type)); //只将对应的ipv4的地址u8*4拷贝过去
  }
  else // AF_INET6
  {
    address_length = sizeof(sockaddr_in6_type); //sock地址结构体28byte
    address.v6.sin6_family = ASIO_OS_DEF(AF_INET6); //协议u16
    address.v6.sin6_port = 0;   //输出地址不需要端口号
    address.v6.sin6_flowinfo = 0;   //
    address.v6.sin6_scope_id = scope_id;    //scope_id是要输出的
    memcpy(&address.v6.sin6_addr, src, sizeof(in6_addr_type)); //最后拷贝ipv6的u8*16
  }

  DWORD string_length = static_cast<DWORD>(length); // 地址长度32/256 bit
#if defined(BOOST_NO_ANSI_APIS) || (defined(_MSC_VER) && (_MSC_VER >= 1800))
  LPWSTR string_buffer = (LPWSTR)_alloca(length * sizeof(WCHAR)); // 使用_alloca分配长度大小的wchar数组【用来装最后输出的字符串的】指针
  ...
  ::WideCharToMultiByte(CP_ACP, 0, string_buffer, -1,
      dest, static_cast<int>(length), 0, 0);  // 将宽字符数组指针对应长度 转化成ansi字符
#else
  ...
  // Windows may set error code on success.【转换成功时也有可能系统会设置异常】
  if (result != socket_error_retval)
    ec.assign(0, ec.category());

  // Windows may not set an error code on failure. 【转换失败时也有可能没有异常】
  else if (result == socket_error_retval && !ec)
    ec = asio::error::invalid_argument;

  return result == socket_error_retval ? 0 : dest;  //【失败返回nullptr成功返回对应的字符串指针】
#else // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // linux下面实现
  const char* result = ::inet_ntop(af, src, dest, static_cast<int>(length)); //这个就很方便，直接inet_ntop就行了【成功返回字符串指针，失败返回null】【成功时】
  get_last_error(ec, true);
  if (result == 0 && !ec)
    ec = asio::error::invalid_argument; //失败设置错误码
  if (result != 0 && af == ASIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
    // 转换成功可能是ipv6地址
    using namespace std; // For strcat and sprintf.
    char if_name[(IF_NAMESIZE > 21 ? IF_NAMESIZE : 21) + 1] = "%"; //设置scope_id输出缓冲数组
    const in6_addr_type* ipv6_address = static_cast<const in6_addr_type*>(src); //将源地址转化成对应的指针
    bool is_link_local = ((ipv6_address->s6_addr[0] == 0xfe) 
        && ((ipv6_address->s6_addr[1] & 0xc0) == 0x80)); //判断是否是本地链路地址[linux下面s6_addr是地址结构 也是u8*16 => fe80::/10 ]
    bool is_multicast_link_local = ((ipv6_address->s6_addr[0] == 0xff)
        && ((ipv6_address->s6_addr[1] & 0x0f) == 0x02));    // 判断是否是链路多播地址[ff02]
    if ((!is_link_local && !is_multicast_link_local)
        || if_indextoname(static_cast<unsigned>(scope_id), if_name + 1) == 0)
        //我们尝试去通过scope_id获取网卡名称
      sprintf(if_name + 1, "%lu", scope_id);    //将if_name从%后面的字符串进行格式化
    strcat(dest, if_name);
  }
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
//【底层函数】将字符串转化成ipv6地址[错误返回-1]
int inet_pton(int af, const char* src, void* dest,
    unsigned long* scope_id, asio::error_code& ec)
{
  clear_last_error();   //先清理以前的错误
#if defined(ASIO_WINDOWS_RUNTIME)
  ...
  int address_length = sizeof(sockaddr_storage_type); //length 128byte
#if defined(BOOST_NO_ANSI_APIS) || (defined(_MSC_VER) && (_MSC_VER >= 1800))
  int num_wide_chars = static_cast<int>(strlen(src)) + 1;   //考虑宽字符，因为strlen不包括"\0"，所以要+1
  LPWSTR wide_buffer = (LPWSTR)_alloca(num_wide_chars * sizeof(WCHAR)); //宽字符的buffer大小，win下要明白宽字符为2byte
  ::MultiByteToWideChar(CP_ACP, 0, src, -1, wide_buffer, num_wide_chars); //将src中的转化成宽字符放进wide_buffer中
  int result = ::WSAStringToAddressW(wide_buffer,
      af, 0, &address.base, &address_length); //用系统函数将这个写进去Address里
  get_last_error(ec, true);
#else
  ...
  if (af == ASIO_OS_DEF(AF_INET))
  {
    if (result != socket_error_retval)  
    {
      memcpy(dest, &address.v4.sin_addr, sizeof(in4_addr_type));    //成功就将转化出来的sin_addr拷贝到dest里面[dest 是 void*]
      ec.assign(0, ec.category());
    }
    else if (strcmp(src, "255.255.255.255") == 0)
    {
      static_cast<in4_addr_type*>(dest)->s_addr = INADDR_NONE; //要是255.255.255.255
      ec.assign(0, ec.category());
    }
  }
  else // AF_INET6
  {
    if (result != socket_error_retval)
    {
      ...
      if (scope_id)
        *scope_id = address.v6.sin6_scope_id; //ipv6的话还要拷贝scope_id
      ec.assign(0, ec.category()); //没错误
    }
  }
  ...
  return result == socket_error_retval ? -1 : 1; //成功返回1，失败返回-1
#else // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  ...
  // 某些平台上inet_pton会在有scope_id的时候失败，先清理掉这个
  const bool is_v6 = (af == ASIO_OS_DEF(AF_INET6)); //判断是不是ipv6
  const char* if_name = is_v6 ? strchr(src, '%') : 0; //是v6就去检索%的位置
  ...
  if (if_name != 0)
  {
    ...
    memcpy(src_buf, src, if_name - src); //将地址字符串拷贝到缓冲区里
    src_buf[if_name - src] = 0; //将缓冲区字符串加个0表示结束
    src_ptr = src_buf; //转换指针
  }

  int result = ::inet_pton(af, src_ptr, dest); //使用inet_pton进行转化
  get_last_error(ec, true);
  if (result <= 0 && !ec)
    ec = asio::error::invalid_argument;
  if (result > 0 && is_v6 && scope_id)
  {
    ...
    *scope_id = 0;
    if (if_name != 0)
    {
      in6_addr_type* ipv6_address = static_cast<in6_addr_type*>(dest); //先将要输出的地址转化成指针in6_addr_type*
      bool is_link_local = ((ipv6_address->s6_addr[0] == 0xfe)
          && ((ipv6_address->s6_addr[1] & 0xc0) == 0x80));  //本地回环 fe80::/10
      bool is_multicast_link_local = ((ipv6_address->s6_addr[0] == 0xff)
          && ((ipv6_address->s6_addr[1] & 0x0f) == 0x02)); //多播 ff02
      if (is_link_local || is_multicast_link_local)
        *scope_id = if_nametoindex(if_name + 1); //这两种就得调用C的方法
      if (*scope_id == 0)
        *scope_id = atoi(if_name + 1); //要不然这个就是纯数字，直接atoi就行
    }
  }
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
int gethostname(char* name, int namelen, asio::error_code& ec)
{
  ...
  int result = ::gethostname(name, namelen);    //[inout char*, in len]
  get_last_error(ec, result != 0);
  return result;
#endif // defined(ASIO_WINDOWS_RUNTIME)
}
