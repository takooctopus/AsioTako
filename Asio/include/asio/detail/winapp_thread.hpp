...
public:
  ...
  {
    scoped_ptr<func_base> arg(new func<Function>(f));   //注意这个func是下面定义的模板类，还有就是new新建堆中对象返回值是指针，所以可以使用指针来新建scoped_ptr对象
    DWORD thread_id = 0;    //thread_id初始化为0，方便后面使用
    thread_ = ::CreateThread(0, 0, winapp_thread_function,
        arg.get(), 0, &thread_id);  //调用api创建thread
    if (!thread_)
    {
      DWORD last_error = ::GetLastError();
      asio::error_code ec(last_error,
          asio::error::get_system_category());
      asio::detail::throw_error(ec, "thread");  //没创建成功就抛异常
    }
    arg.release();
  }
  ...
  ~winapp_thread()
  {
    ::CloseHandle(thread_); //调用api销毁线程
  }

  // Wait for the thread to exit.
  void join()
  {
    ::WaitForSingleObjectEx(thread_, INFINITE, false);  //阻塞join
  }

  // Get number of CPUs.
  static std::size_t hardware_concurrency() //获取cpu核心数
  {
...
private:
  friend DWORD WINAPI winapp_thread_function(LPVOID arg);   //声明友元函数

  class func_base   //函数抽象接口
  {
  public:
    virtual ~func_base() {}
    virtual void run() = 0;
  ...
  template <typename Function>
  class func
    : public func_base  //用来被scpoed_ptr管理生命周期的函数对象
  {
  public:
    func(Function f)
      : f_(f)
