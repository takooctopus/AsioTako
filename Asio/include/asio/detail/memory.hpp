...
#include <cstdlib> //【字符串转换为数字的函数, 随机数函数, 内存分配释放函数, 程序运行控制函数, 系统访问相关函数, 常用算法函数】
#include <memory>   //【三种ptr，default_delete， allocator， allocator_traits ...】
#include <new> 
...
#if defined(ASIO_HAS_STD_SHARED_PTR)
using std::make_shared; //标准导入make_shared
using std::shared_ptr;  //标准导入shared_ptr
using std::weak_ptr;    //标准导入weak_ptr
...
#if defined(ASIO_HAS_STD_ADDRESSOF)
using std::addressof;   //标准导入addressof，一般的&定义就是这个
